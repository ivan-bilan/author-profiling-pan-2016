...
class FinalClassicationClass(object):
    """
    Create the classification pipeline depending on the input language.
    """
    ...
    def make_pipeline_en(self, classifier, comment="", lang=None):
        ...
        pipeline2 = Pipeline([
            ...
            # turn RFE ON by uncommenting the following line
    ...
    def clf_custom_score(self, y_dev, y_pred, comment=""):
        ...
        """
        Calculates various score metrics based on the final classification

        # to calculate score based on samples, call the function like this:
        comment = "gender per sample"
        y_pred_gender = pipeline_gender.predict(list_of_dict_dev)
        clf_custom_score(dev_list_of_labels_gender, y_pred_gender, comment)

        # to calculate score based on authors, call the function like this:
        ...
        new_comment = comment + " gender per author"
        clf_custom_score(newlist_dev_gender, newlist_pred_gender, new_comment)
        """
        ...
        for key, value in dev_index_for_each_author.iteritems():
            ...
            for index, element in enumerate(dev_list_of_labels_age):
                ...
                if index in value:
                    ...
                    elif dev_list_of_labels_age[index] == 7:
                        count_64.append(dev_list_of_labels_age[index])
            # print count_male

            max_value = max(len(count_18), len(count_25), len(count_35), len(count_50), len(count_64),)
            # print max_value, type(max_value)

            if max_value == len(count_18) and max_value != len(count_25) and max_value != len(count_35) and max_value != len(count_50) and max_value != len(count_64):
                final_tr_dict[key] = 3
            elif max_value != len(count_18) and max_value == len(count_25) and max_value != len(count_35) and max_value != len(count_50) and max_value != len(count_64):
                final_tr_dict[key] = 4
            elif max_value != len(count_18) and max_value != len(count_25) and max_value == len(count_35) and max_value != len(count_50) and max_value != len(count_64):
                final_tr_dict[key] = 5
            elif max_value != len(count_18) and max_value != len(count_25) and max_value != len(count_35) and max_value == len(count_50) and max_value != len(count_64):
    ...
    def get_author_classes_gender_evaluator(self, dev_authors, dev_list_of_labels_gender, value):
        if value == 2:
            dev_index_for_each_author = dict()
            for index, element in enumerate(dev_authors):
            ...
            for index, element in enumerate(dev_authors):
                if element in dev_list_of_labels_gender:
                    final_tr_dict[element] = dev_list_of_labels_gender[element][0]
    ...
    def get_author_classes_age_evaluator(self, dev_authors, dev_list_of_labels_age, value):
        ...
        if value == 2:
            ...
            for key, value in dev_index_for_each_author.iteritems():
                ...
                max_value = max(len(count_18), len(count_35), len(count_50))
                # print max_value, type(max_value)

                if max_value == len(count_18) and max_value != len(count_35) and max_value != len(count_50):
                    final_tr_dict[key] = 3
                elif max_value != len(count_18) and max_value == len(count_35) and max_value != len(count_50):
                    final_tr_dict[key] = 4
                elif max_value != len(count_18) and max_value != len(count_35) and max_value == len(count_50):
                    final_tr_dict[key] = 5
                elif max_value == len(count_18) and max_value == len(count_35) and max_value != len(count_50):
                    final_tr_dict[key] = randint(3,4)
                    # print "random1"
                elif max_value != len(count_18) and max_value == len(count_35) and max_value == len(count_50):
                    final_tr_dict[key] = randint(4,5)
                    # print "random2"
                elif max_value == len(count_18) and max_value != len(count_35) and max_value == len(count_50):
                    # print "random3"
                    final_tr_dict[key] = randint(3,5)
                elif max_value == len(count_18) and max_value == len(count_35) and max_value == len(count_50):
                    # print "random3"
                    final_tr_dict[key] = randint(3,5)
                else:
                    print "Error, no age classified!"
                    final_tr_dict[key] = randint(3,5)
            ...
            for index, element in enumerate(dev_authors):
                if element in dev_list_of_labels_age:
                    final_tr_dict[element] = dev_list_of_labels_age[element][1]
    ...
    def generate_final_xml(self, output_folder, language, predicted_gender, predicted_age=None):
        ...
        """
        Generates final xml files with the classified labels.
        """
        ...
        if predicted_age is None:
            ...
            for author in predicted_gender:
                ...
                if predicted_age[author] == 3:
                    ...
                    current_age_group = "50-64"
                elif predicted_age[author] == 7:
                    current_age_group = "65-xx"
    ...
    def dataset_statistics(self, X, y_gender, y_author, lang, y_age=None):
        """
        Output class distribution of the dataset.
        """
        print
        print "Final Dataset Statistics"
        print "Number of Text Samples: ", len(X)
        ...
        if y_age is None:
            ...
            print "Number of 50-64 Age Group Text Samples: ", y_age.count(6)
